...
export default function Thoughts() {
  ...
  return (
    <main className="pm-main">
      <header className="pm-page-header"><h1>闲言</h1></header>
            ...
            {thoughts.map((t, i) => (
              <article key={i} className="pm-moment-item">
                ...
                <div className="pm-moment-body">
                  <div className="pm-moment-author">{siteConfig.headerTitle}</div>
                  <div className="pm-moment-text">{t.content}</div>
                  {t.images.length > 0 && (
                    <div className={`pm-moment-gallery pm-moment-gallery-n${Math.min(t.images.length, 9)}`}>
                      {t.images.slice(0, 9).map((img, j) => (
                        <img key={j} src={img} alt="" loading="lazy" decoding="async" fetchPriority="low" />
                      ))}
                    </div>
                  )}
...
function MomentTime({ dateStr }: { dateStr: string }) {
  ...
  useEffect(() => {
    ...
    // Re-check every 60s for relative time updates
